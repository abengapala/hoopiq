...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`${BACKEND}/players/search?q=${encodeURIComponent(search.trim())}&limit=4`)
        if (res.ok) {
  ...
  // Close mobile search on nav
  useEffect(() => {
    if (!searchOpen) { setSearch(''); setPlayers([]) }
  }, [searchOpen])

  const showDropdown = (searchFocus || searchOpen) && search.trim().length >= 1
  const allResults = [
  ...
  const handleKeyDown = (e) => {
    ...
    else if (e.key === 'Escape')   { setSearchFocus(false); setSearchOpen(false); inputRef.current?.blur() }
  }

  const SearchDropdown = () => (
    <div ref={dropdownRef} style={{
      position: isMobile ? 'fixed' : 'absolute',
      top: isMobile ? 64 : 'calc(100% + 6px)',
      ...
      {teamResults.length > 0 && (
        <div>
          <div style={{ padding: '8px 16px 4px', fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', color: 'var(--text3)', textTransform: 'uppercase' }}>
            Teams
          </div>
          {teamResults.map((team, i) => (
            <div key={team.id} onMouseDown={() => handleSelect({ type: 'team', ...team })} onMouseEnter={() => setActiveIndex(i)}
              style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', cursor: 'pointer', background: activeIndex === i ? 'var(--bg3)' : 'transparent', transition: 'background 0.1s' }}>
              <TeamLogoSmall abbr={team.abbr} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, color: 'var(--text1)', fontWeight: 500 }}>{team.name}</div>
                <div style={{ fontSize: 11, color: 'var(--text3)' }}>{team.abbr}</div>
              </div>
              <div style={{ fontSize: 10, color: 'var(--accent)', background: 'rgba(59,130,246,0.1)', padding: '2px 6px', borderRadius: 4, fontWeight: 600 }}>TEAM</div>
            </div>
          ))}
        </div>
      )}
          ...
          {players.map((player, i) => {
            ...
            return (
              <div key={player.id} onMouseDown={() => handleSelect({ type: 'player', ...player })} onMouseEnter={() => setActiveIndex(gi)}
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', cursor: 'pointer', background: activeIndex === gi ? 'var(--bg3)' : 'transparent', transition: 'background 0.1s' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--bg3)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--accent)', flexShrink: 0 }}>
                  {(player.name || '?')[0].toUpperCase()}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, color: 'var(--text1)', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{player.name}</div>
                  <div style={{ fontSize: 11, color: 'var(--text3)' }}>{player.team}{player.position ? ` · ${player.position}` : ''}</div>
                </div>
                <div style={{ fontSize: 10, color: 'rgba(34,197,94,0.9)', background: 'rgba(34,197,94,0.1)', padding: '2px 6px', borderRadius: 4, fontWeight: 600, flexShrink: 0 }}>PLAYER</div>
              </div>
            )
          })}
        </div>
      ...
      {playerLoading && players.length === 0 && (
        <div style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text3)', fontSize: 13 }}>
      ...
      {search.trim().length === 1 && teamResults.length === 0 && (
        <div style={{ padding: '16px', color: 'var(--text3)', fontSize: 12, textAlign: 'center' }}>Keep typing to search players...</div>
      )}

      {!isMobile && allResults.length > 0 && (
    ...
    return (
      <>
        <nav style={{
          ...
          {/* Accent line */}
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(90deg, var(--accent), rgba(59,130,246,0.3) 60%, transparent)', opacity: 0.8 }} />

          {/* Logo */}
          <NavLink to="/" style={{ textDecoration: 'none', flexShrink: 0, display: 'flex', alignItems: 'center' }}>
            <img src={logo} alt="HoopIQ" style={{ height: 44, width: 'auto', objectFit: 'contain' }} />
          </NavLink>
          ...
          {searchOpen ? (
            <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
              <svg style={{ position: 'absolute', left: 10, color: 'var(--accent)', pointerEvents: 'none' }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <input
                ...
                autoFocus
                className="input"
                style={{ width: '100%', fontSize: 14, height: 38, padding: '0 12px 0 34px' }}
          ...
          <button
            ...
            style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid var(--border)', background: searchOpen ? 'var(--accent-bg)' : 'var(--bg3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: searchOpen ? 'var(--accent)' : 'var(--text3)', flexShrink: 0 }}
          >
            {searchOpen
          ...
          <button onClick={toggle}
            style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', flexShrink: 0 }}
          >
            {dark
        ...
        {/* Full-screen search overlay results */}
        {searchOpen && search.trim().length >= 1 && <SearchDropdown />}

        {/* Backdrop to close search */}
        {searchOpen && search.trim().length === 0 && (
          <div onClick={() => setSearchOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 190, top: 64 }} />
        )}
      ...
      <div className="page-wrapper" style={{ width: '100%', display: 'flex', alignItems: 'center', paddingTop: 0, paddingBottom: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginLeft: 12 }}>
          ...
          <div style={{ position: 'relative' }}>
            <svg style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: searchFocus ? 'var(--accent)' : 'var(--text3)', pointerEvents: 'none', transition: 'color 0.15s', zIndex: 1 }} width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
            </svg>
            <input
              ref={inputRef}
