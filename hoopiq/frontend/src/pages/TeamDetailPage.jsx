...
const ABBR_MAP = {
  GS:'GSW', NO:'NOP', NOH:'NOP', NOK:'NOP',
  SA:'SAS', NY:'NYK', OK:'OKC',
...
export default function TeamDetailPage() {
  ...
  const { team, stats, roster, last5, upcoming, streak } = data
  const rawAbbr      = team.abbreviation || ''
  const abbr         = norm(rawAbbr)           // normalize: GS → GSW, SA → SAS, etc.
  const primaryColor = TEAM_COLORS[abbr] || '#888'
  const champ        = CHAMPIONSHIPS[abbr]     // now correctly finds GSW, SAS, NYK, etc.
          ...
          <div className="card" style={{ marginBottom: 20, padding: 'clamp(16px, 4vw, 24px)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 14 }}>
              <div style={{
                ...
                lineHeight: 1, color: '#FDB927', letterSpacing: '0.02em',
              }}>
                {champ.titles}
              </div>
              <div>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 20 }}>
            <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
              <div style={{ minWidth: 340 }}>
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: '100px 1fr 80px 44px',
                  padding: '10px 16px',
                  borderBottom: '1px solid var(--border)',
                  background: 'var(--bg3)',
                }}>
                  {['Date', 'Matchup', 'Score', 'W/L'].map((h, idx) => (
                ...
                {last5.map((g, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{
                      ...
                      {g.PTS && g.OPP_PTS ? `${g.PTS}–${g.OPP_PTS}` : '—'}
                    </span>
                    <span style={{
              ...
              {upcoming.map((g, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <TeamLogo abbr={g.OPP_ABBR} size={28} />
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>
                        {g.HOME_AWAY} {g.OPP_FULL || g.OPP_ABBR}
                      </div>
                      {g.VENUE && (
                        <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{g.VENUE}</div>
                      )}
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 12, fontFamily: 'DM Mono, monospace', color: 'var(--text2)' }}>
